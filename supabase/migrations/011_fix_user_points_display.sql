-- ============================================
-- Fix User Points Display Issue
-- ============================================
-- Replace static pr.points with dynamic calculation
-- ============================================

-- Update get_posts_with_stats to use dynamic points calculation
...
-- Update get_user_shared_posts to use dynamic points calculation
...
-- Update get_user_posts to use dynamic points calculation
...
-- Update get_post_with_stats to use dynamic points calculation  
...
-- ============================================
-- Product RPC Functions
-- ============================================

-- Function to get products with seller stats
...
BEGIN
    ...
    SELECT 
        ...
        COALESCE(calculate_user_points(prod.user_id), 0) AS seller_points
    FROM public.products prod
    LEFT JOIN public.profiles pr ON prod.user_id = pr.id
    WHERE 
        (category_filter IS NULL OR prod.category = category_filter)
        AND (search_query IS NULL OR prod.name ILIKE '%' || search_query || '%' OR prod.description ILIKE '%' || search_query || '%')
...
-- Function to get single product with seller stats
